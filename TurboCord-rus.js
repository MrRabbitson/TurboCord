/*!
 * TurboCord от MrRabbitson
 */

(function (Scratch) {
  'use strict';

  const vm = Scratch.vm;
  const runtime = vm.runtime;

  let experimentsDisabled = false;
  let webhookHidden = true;
  let embedHidden = true;
  let discordIcon = 'data:image/png;base64,';
  let webhookIcon = 'data:image/png;base64,';
  let embedIcon = 'data:image/png;base64,';


  const label = (name, hidden) => ({
    blockType: Scratch.BlockType.LABEL,
    text: name,
    hideFromPalette: hidden
  });

  async function createNewWebhook() {
    let name = prompt('Введите имя вебхука');
    if (!name) {
      return;
    }
    let url = prompt('Введите URL вебхука');
    if (!url) {
      return;
    }
    const webhookData = await Scratch.fetch(url);
    const jsonData = await webhookData.json();
    const webhookName = jsonData['name'];
    webhooks[name] = {URL: url, USERNAME: webhookName, AVATAR: ''};
    webhookHidden = false;
    Scratch.vm.extensionManager.refreshBlocks();
  }

  function createNewEmbed() {
    let name = prompt('Введите имя эмбеда');
    if (!name) {
      return;
    }
    embeds[name] = {
      "title": "Заголовок",
      "description": "Описание",
      "url": null,
      "color": null,
      "author": null,
      "image": {"url": null},
      "thumbnail": {"url": null}};
    embedHidden = false;
    Scratch.vm.extensionManager.refreshBlocks();
  }

  let webhooks = Object.create(null);
  let embeds = Object.create(null);

  class DiscordAPI {
    getInfo() {
      return {
        id: 'rabbitdiscordapi',
        name: 'TurboCord by MrRabbitson',
        color1: '#586aea',
        color2: '#4b5ad2',
        color3: '#3747bd',
        menuIconURI: discordIcon,
        blocks: [
          {
            opcode: 'sendBasicMessage',
            blockType: Scratch.BlockType.COMMAND,
            text: 'отправить [TYPE] [STRING] по URL [WEBHOOK]',
            arguments: {
              TYPE: {
                type: Scratch.ArgumentType.STRING,
                menu: 'sendTypeMenu',
                defaultValue: 'text'
              },
              STRING: {
                type: Scratch.ArgumentType.STRING,
                defaultValue: 'Привет!'
              },
              WEBHOOK: {
                type: Scratch.ArgumentType.STRING
              }
            }
          },
          {
            func: 'enableExperimental',
            blockType: Scratch.BlockType.BUTTON,
            text: 'Включить экспериментальные функции',
            hideFromPalette: !experimentsDisabled
          },

          '---',

          label('Вебхуки', experimentsDisabled),

          {
            opcode: 'addWebhook',
            blockType: Scratch.BlockType.BUTTON,
            text: 'Добавить вебхук',
            func: 'addWebhook',
            disableMonitor: true,
            hideFromPalette: experimentsDisabled
          },
          {
            opcode: 'deleteWebhook',
            blockType: Scratch.BlockType.BUTTON,
            text: 'Удалить вебхук',
            func: 'deleteWebhook',
            disableMonitor: true,
            hideFromPalette: webhookHidden
          },
          {
            opcode: 'setWebhookAttribute',
            blockType: Scratch.BlockType.COMMAND,
            blockIconURI: webhookIcon,
            text: 'установить [ATTRIBUTE] для [WEBHOOK] в [STRING]',
            arguments: {
              ATTRIBUTE: {
                type: Scratch.ArgumentType.STRING,
                menu: 'webhookAttributeMenu'
              },
              WEBHOOK: {
                type: Scratch.ArgumentType.STRING,
                menu: 'webhookMenu'
              },
              STRING: {
                type: Scratch.ArgumentType.STRING,
                defaultValue: 'MrRabbitson'
              }
            },
            hideFromPalette: webhookHidden
          },
          {
            opcode: 'sendMessage',
            blockType: Scratch.BlockType.COMMAND,
            blockIconURI: webhookIcon,
            text: 'отправить [TYPE] [STRING] в [WEBHOOK] эмбед [EMBED]',
            arguments: {
              TYPE: {
                type: Scratch.ArgumentType.STRING,
                menu: 'sendTypeMenu',
                defaultValue: 'text'
              },
              STRING: {
                type: Scratch.ArgumentType.STRING,
                defaultValue: 'Привет!'
              },
              WEBHOOK: {
                type: Scratch.ArgumentType.STRING,
                menu: 'webhookMenu'
              },
              EMBED: {
                type: Scratch.ArgumentType.STRING,
                menu: 'embedMenu'
              }
            },
            hideFromPalette: webhookHidden
          },
          {
            opcode: 'getWebhookAttribute',
            blockType: Scratch.BlockType.REPORTER,
            blockIconURI: webhookIcon,
            text: '[ATTRIBUTE] для [WEBHOOK]',
            arguments: {
              ATTRIBUTE: {
                type: Scratch.ArgumentType.STRING,
                menu: 'webhookAttributeMenu'
              },
              WEBHOOK: {
                type: Scratch.ArgumentType.STRING,
                menu: 'webhookMenu'
              }
            },
            hideFromPalette: webhookHidden
          },

          '---',

          label('Эмбеды', experimentsDisabled),

          {
            opcode: 'createEmbed',
            blockType: Scratch.BlockType.BUTTON,
            text: 'Создать эмбед',
            func: 'createEmbed',
            disableMonitor: true,
            hideFromPalette: experimentsDisabled
          },
          {
            opcode: 'deleteEmbed',
            blockType: Scratch.BlockType.BUTTON,
            text: 'Удалить эмбед',
            func: 'deleteEmbed',
            disableMonitor: true,
            hideFromPalette: embedHidden
          },
          {
            opcode: 'getEmbedAttribute',
            blockType: Scratch.BlockType.REPORTER,
            blockIconURI: embedIcon,
            text: '[ATTRIBUTE] эмбеда [EMBED]',
            arguments: {
              ATTRIBUTE: {
                type: Scratch.ArgumentType.STRING,
                menu: 'embedAttributeMenu'
              },
              EMBED: {
                type: Scratch.ArgumentType.STRING,
                menu: 'embedMenu'
              }
            },
            hideFromPalette: embedHidden
          },

          '---',

          {
            opcode: 'setEmbedAttribute',
            blockType: Scratch.BlockType.COMMAND,
            blockIconURI: embedIcon,
            text: 'установить [ATTRIBUTE] для [EMBED] в [STRING]',
            arguments: {
              ATTRIBUTE: {
                type: Scratch.ArgumentType.STRING,
                menu: 'embedAttributeMenu'
              },
              EMBED: {
                type: Scratch.ArgumentType.STRING,
                menu: 'embedMenu'
              },
              STRING: {
                type: Scratch.ArgumentType.STRING,
                defaultValue: 'Объявление'
              }
            },
            hideFromPalette: embedHidden
          },
          {
            opcode: 'setEmbedColor',
            blockType: Scratch.BlockType.COMMAND,
            blockIconURI: embedIcon,
            text: 'установить цвет для [EMBED] в [COLOR]',
            arguments: {
              EMBED: {
                type: Scratch.ArgumentType.STRING,
                menu: 'embedMenu'
              },
              COLOR: {
                type: Scratch.ArgumentType.COLOR,
                defaultValue: '#5ee59d'
              }
            },
            hideFromPalette: embedHidden
          },
          {
            opcode: 'setEmbedAuthor',
            blockType: Scratch.BlockType.COMMAND,
            blockIconURI: embedIcon,
            text: 'установить автора [ATTRIBUTE] для [EMBED] в [STRING]',
            arguments: {
              ATTRIBUTE: {
                type: Scratch.ArgumentType.STRING,
                menu: 'authorMenu'
              },
              EMBED: {
                type: Scratch.ArgumentType.STRING,
                menu: 'embedMenu'
              },
              STRING: {
                type: Scratch.ArgumentType.STRING,
              }
            },
            hideFromPalette: embedHidden
          }
        ],
        menus: {
          webhookMenu: {
            acceptReporters: true,
            items: 'createWebhookMenu'
          },
          embedMenu: {
            acceptReporters: true,
            items: 'createEmbedMenu'
          },
          sendTypeMenu: {
            acceptReporters: false,
            items: [
              {
                text: 'текст',
                value: 'text'
              },
              {
                text: 'json',
                value: 'json'
              }
            ]
          },
          webhookAttributeMenu: {
            acceptReporters: false,
            items: [
              {
                text: 'имя пользователя',
                value: 'username'
              },
              {
                text: 'аватар URL',
                value: 'avatar'
              },
              {
                text: 'URL',
                value: 'url'
              }
            ]
          },
          embedAttributeMenu: {
            acceptReporters: false,
            items: [
              {
                text: 'заголовок',
                value: 'title'
              },
              {
                text: 'описание',
                value: 'description'
              },
              {
                text: 'URL',
                value: 'url'
              },
              {
                text: 'URL миниатюры',
                value: 'thumburl'
              },
              {
                text: 'URL изображения',
                value: 'imageurl'
              }
            ]
          },
          stateMenu: {
            acceptReporters: false,
            items: [
              {
                text: 'включено',
                value: 'enabled'
              },
              {
                text: 'выключено',
                value: 'disabled'
              }
            ]
          },
          authorMenu: {
            acceptReporters: false,
            items: [
              {
                text: 'имя',
                value: 'name'
              },
              {
                text: 'URL',
                value: 'url'
              },
              {
                text: 'URL иконки',
                value: 'iconurl'
              }
            ]
          },
        }
      };
    }

    sendBasicMessage(args) {
      if (!args.STRING || !args.WEBHOOK) {
        return;
      }
      const webhookURL = args.WEBHOOK;
      let content = '';
      if (args.TYPE === 'text') {
        let mappedEmbeds = [];
        if (embeds[args.EMBED]) {
          mappedEmbeds = [embeds[args.EMBED]];
        }
        let defaultData = {
          content: args.STRING
        };
        content = JSON.stringify(defaultData);
      } else {
        content = args.STRING;
      }
      try {
        Scratch.fetch(webhookURL, {
          method: "POST",
          headers: {
            'Content-type': 'application/json'
          },
          body: content
        });
      } catch (error) {
        return '';
      }
    }

    enableExperimental() {
      let confirmMsg = 'Внимание: Хотя эти функции полностью работоспособны, пользовательские вебхуки/эмбеды в настоящее время не сохраняются в проектах. \n \n Продолжить?';
      if (confirm(confirmMsg)) {
        experimentsDisabled = false;
        Scratch.vm.extensionManager.refreshBlocks();
      }
    }

    addWebhook() {
      createNewWebhook();
    }

    deleteWebhook() {
      let name = prompt('Введите имя вебхука');
      if (!name) {
        return;
      }
      Reflect.deleteProperty(webhooks, name);
      Scratch.vm.extensionManager.refreshBlocks();
      if (Object.keys(webhooks).length == 0) {
        webhookHidden = true;
        Scratch.vm.extensionManager.refreshBlocks();
      }
    }

    setWebhookAttribute(args) {
      if (!webhooks[args.WEBHOOK]) {
        return;
      }
      if (args.ATTRIBUTE === 'url') {
        webhooks[args.WEBHOOK]['URL'] = args.STRING;
      } else if (args.ATTRIBUTE === 'avatar') {
        webhooks[args.WEBHOOK]['AVATAR'] = args.STRING;
      } else {
        webhooks[args.WEBHOOK]['USERNAME'] = args.STRING;
      }
    }

    sendMessage(args) {
      if (!webhooks[args.WEBHOOK]) {
        return;
      }
      const webhookURL = webhooks[args.WEBHOOK]['URL'];
      const webhookUsername = webhooks[args.WEBHOOK]['USERNAME'];
      const webhookAvatar = webhooks[args.WEBHOOK]['AVATAR'];
      let content = '';
      if (args.TYPE === 'text') {
        let mappedEmbeds = [];
        if (embeds[args.EMBED]) {
          mappedEmbeds = [embeds[args.EMBED]];
        }
        let defaultData = {
          username: webhookUsername,
          avatar_url: webhookAvatar,
          content: args.STRING,
          embeds: mappedEmbeds};
        content = JSON.stringify(defaultData);
      } else {
        content = args.STRING;
      }
      try {
        Scratch.fetch(webhookURL, {
          method: "POST",
          headers: {
            'Content-type': 'application/json'
          },
          body: content
        });
      } catch (error) {
        return '';
      }
    }

    getWebhookAttribute(args) {
      if (!webhooks[args.WEBHOOK]) {
        return '';
      }
      if (args.ATTRIBUTE === 'url') {
        return webhooks[args.WEBHOOK]['URL'];
      } else if (args.ATTRIBUTE === 'avatar') {
        return webhooks[args.WEBHOOK]['AVATAR'];
      } else {
        return webhooks[args.WEBHOOK]['USERNAME'];
      }
    }

    createEmbed() {
      createNewEmbed();
    }

    deleteEmbed() {
      let name = prompt('Введите имя эмбеда');
      if (!name) {
        return;
      }
      Reflect.deleteProperty(embeds, name);
      Scratch.vm.extensionManager.refreshBlocks();
      if (Object.keys(embeds).length == 0) {
        embedHidden = true;
        Scratch.vm.extensionManager.refreshBlocks();
      }
    }

    setEmbedAttribute(args) {
      if (!embeds[args.EMBED]) {
        return;
      }
      if (args.ATTRIBUTE === 'title') {
        embeds[args.EMBED]['title'] = args.STRING;
      } else if (args.ATTRIBUTE === 'description') {
        embeds[args.EMBED]['description'] = args.STRING;
      } else if (args.ATTRIBUTE === 'url') {
        embeds[args.EMBED]['url'] = args.STRING;
      } else if (args.ATTRIBUTE === 'thumburl') {
        embeds[args.EMBED]['thumbnail']['url'] = args.STRING;
      } else if (args.ATTRIBUTE === 'imageurl') {
        embeds[args.EMBED]['image']['url'] = args.STRING;
      }
      return '';
    }

    setEmbedColor(args) {
      if (!embeds[args.EMBED]) {
        return '';
      }
      let color = args.COLOR;
      color = color.substring(1, 7);
      color = Scratch.Cast.toNumber("0x" + color);
      embeds[args.EMBED]['color'] = color;
    }

    setEmbedAuthor(args) {
      if (!embeds[args.EMBED]) {
        return '';
      }
      if (!embeds[args.EMBED]['author']) {
        embeds[args.EMBED]['author'] = {"name": "Имя автора"};
      }
      if (args.ATTRIBUTE === 'name') {
        embeds[args.EMBED]['author']['name'] = args.STRING;
      } else if (args.ATTRIBUTE === 'url') {
        embeds[args.EMBED]['author']['url'] = args.STRING;
      } else if (args.ATTRIBUTE === 'iconurl') {
        embeds[args.EMBED]['author']['icon_url'] = args.STRING;
      }
      return '';
    }

    getEmbedAttribute(args) {
      if (!embeds[args.EMBED]) {
        return '';
      }
      if (args.ATTRIBUTE === 'title') {
        return embeds[args.EMBED]['title'];
      } else if (args.ATTRIBUTE === 'description') {
        return embeds[args.EMBED]['description'];
      } else if (args.ATTRIBUTE === 'url') {
        return embeds[args.EMBED]['url'];
      }
    }

    createWebhookMenu() {
      const webhookMenu = [];
      const length = Object.keys(webhooks).length;
      for (let index = 0; index < length; index++) {
        let name = Object.keys(webhooks)[index];
        webhookMenu.push({
          text: name,
          value: name
        });
      }
      const newWebhook = {text: 'Новый вебхук', value: () => createNewWebhook()};
      if (webhookMenu.length > 0) {
        webhookMenu.push(newWebhook);
        return webhookMenu;
      } else {
        return [{text: '', value: ''}, newWebhook];
      }
    }

    createEmbedMenu() {
      const embedMenu = [];
      const length = Object.keys(embeds).length;
      for (let index = 0; index < length; index++) {
        let name = Object.keys(embeds)[index];
        embedMenu.push({
          text: name,
          value: name
        });
      }
      const newEmbed = {text: 'Новый эмбед', value: () => createNewEmbed()};
      const noEmbed = {text: 'без эмбеда', value: 'no embed'};
      if (embedMenu.length > 0) {
        embedMenu.push(noEmbed);
        embedMenu.push(newEmbed);
        return embedMenu;
      } else {
        return [noEmbed, newEmbed];
      }
    }
  }

  Scratch.extensions.register(new DiscordAPI());
})(Scratch);